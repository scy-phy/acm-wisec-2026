...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", required=True, help="toc.html | wisec21-authors.csv")
...
''')
    ...
    if args.data == "toc.html":
        toc, _ = parse_toc("toc.html")
        number = 0
        for entry in toc:
            title = entry[0]
            acm_url = entry[1]
            authors = entry[2]
            abstract = ''.join(entry[3:])
            mid = title.replace(' ','_')
            author = " and ".join(a for a in authors)

            print(f'''<p><div class="publication-entry" id="{mid}">
    <span class="title"><a class="publication-link" href="#{mid}"><i class="icon fas fa-link"></i></a>{title}</span><div class="author">{author}
        </div>
    <div class="publication-buttons"><a class="btn btn-outline-primary" role="button" href="{acm_url}"
           target="_blank" rel="noopener"><i class="iconify" data-icon="cib-acm"></i><span class="label">&nbsp;&nbsp;Open Access</span></a></div></div>

<div class="spoiler " >
<p>
  <a class="btn btn-primary" data-toggle="collapse" href="#spoiler-{number}" role="button" aria-expanded="false" aria-controls="spoiler-{number}">
    Show abstract
  </a>
</p>
<div class="collapse " id="spoiler-{number}">
  <div class="card">
    <div class="card-body">
{abstract}
    </div>
